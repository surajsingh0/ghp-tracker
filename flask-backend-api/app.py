...
# Configuration
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///ghp_tracker.db'
...
@app.route('/ghp', methods=['GET', 'POST'])
@jwt_required()
def handle_ghp():
    user_id = get_jwt_identity()
...
@app.route('/ghp/<int:ghp_id>', methods=['DELETE'])
@jwt_required()
...
@app.route('/ghp/<int:ghp_id>/check', methods=['POST'])
@jwt_required()
...
@app.route('/checked_days/<int:ghp_id>/<string:date>/notes', methods=['POST', 'GET', 'PUT'])
@jwt_required()
